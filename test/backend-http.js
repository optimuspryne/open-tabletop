...
test('asyncRoute forwards rejected promises to Express next', async () => {
  ...
  const wrapped = asyncRoute(async () => { throw expected; });
...
test('HTTP error boundary returns a generic 500 response', () => {
  ...
  console.error = () => {};
  try {
    httpErrorHandler(new Error('secret database detail'), { method: 'GET', originalUrl: '/rooms' }, response, () => {});
  } finally {
...
test('bearer authentication hashes the token and returns its user', async () => {
  const db = { findUserByToken: async (digest) => digest === 'hash:raw-token' ? { id: '7' } : null };
  const requireUser = createRequireUser({ db, hashToken: (token) => `hash:${token}` });
  const response = { status() { throw new Error('unexpected rejection'); } };
  assert.deepEqual(await requireUser({ headers: { authorization: 'Bearer raw-token' } }, response), { id: '7' });
});

test('bearer authentication rejects missing and non-Bearer credentials', async () => {
  const db = { findUserByToken: async () => { throw new Error('DB must not be queried without a token'); } };
...
test('public user shape never exposes authentication hashes', () => {
  const result = clientUser({ id: '1', username: 'player', email: 'p@example.com', avatar: '', isAdmin: false,
    canOwnRooms: false, hostStatus: 'none', hasPassword: true, passwordHash: 'secret', loginTokenHash: 'secret' });
