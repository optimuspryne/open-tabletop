...
test('chess: 32 valid pieces, 16 per side, one per square', () => {
  ...
  assert.ok(p.every(x => PROPS[x.shape] && PROPS[x.shape].team === 'chess'), 'every shape is a chess-team piece');
  assert.ok(p.every(x => x.col >= 0 && x.col < 8 && x.row >= 0 && x.row < 8), 'all on the board');
...
test('checkers: 24 pieces, 12 per side, dark squares only, none overlapping', () => {
  ...
  assert.ok(p.every(x => (x.col + x.row) % 2 === 1), 'dark squares only');
  assert.ok(p.every(x => x.shape === 'checker'), 'all checkers');
  assert.ok(p.every(x => x.row < 3 || x.row > 4), 'the middle two rows are empty');
...
test('dominoes: a domino set dealt to hands', () => {
  ...
  assert.ok(STARTER_LIST.some(g => g.id === 'dominoes'), 'listed in the Games tab');
});
...
test('the go board pins its printed-line spacing (bordered grid, so it can\'t derive from the collider)', () => {
  ...
  assert.ok(grid.cellX < (BOARDS.go.box[0] * 2) / grid.cells, 'pinned cellX is inside the collider-derived value');
  assert.ok(grid.cellZ < (BOARDS.go.box[2] * 2) / grid.cells, 'pinned cellZ is inside the collider-derived value');
});
