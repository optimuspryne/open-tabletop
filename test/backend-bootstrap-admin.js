...
test('admin bootstrap is disabled when no provisioning variables are present', async () => {
  ...
  const result = await bootstrapAdminFromEnvironment({
    db: { bootstrapAdmin: async () => { called = true; } },
...
test('partial bootstrap configuration fails closed', async () => {
  await assert.rejects(() => bootstrapAdminFromEnvironment({
    db: {}, hashPassword: async () => 'hash', env: { BOOTSTRAP_ADMIN_USERNAME: 'admin' },
  }), /requires BOOTSTRAP_ADMIN_USERNAME/);
});

test('bootstrap validates identity and requires a strong password', async () => {
  await assert.rejects(() => bootstrapAdminFromEnvironment({
    db: {}, hashPassword: async () => 'hash', env: { ...validEnv, BOOTSTRAP_ADMIN_USERNAME: 'bad name' }, readFile: () => 'long-enough-password',
  }), /USERNAME/);
  await assert.rejects(() => bootstrapAdminFromEnvironment({
    db: {}, hashPassword: async () => 'hash', env: { ...validEnv, BOOTSTRAP_ADMIN_EMAIL: 'invalid' }, readFile: () => 'long-enough-password',
  }), /EMAIL/);
  await assert.rejects(() => bootstrapAdminFromEnvironment({
    db: {}, hashPassword: async () => 'hash', env: validEnv, readFile: () => 'too-short',
  }), /at least 12/);
});
...
test('bootstrap reads the secret file, hashes it, and passes no plaintext to the database', async () => {
  ...
  let provisioned;
  const result = await bootstrapAdminFromEnvironment({
    db: { bootstrapAdmin: async (record) => { provisioned = record; return { status: 'created' }; } },
    hashPassword: async (password) => { hashed = password; return 'password-hash'; },
    env: validEnv,
  ...
  assert.deepEqual(provisioned, { username: 'site_admin', email: 'admin@example.com', passwordHash: 'password-hash' });
