...
test('snapToCell (rectangular): X and Z snap on independent spacings (go board)', () => {
  ...
  assert.deepEqual(snapToCell(0.30, 0.30, sc), { x: 0.4, z: 0.5 }); // different spacing pulls x and z apart
  // cellZ ≤ 0 falls back to the width (square)
  assert.deepEqual(snapToCell(0.79, 0.79, { gridStyle: 'square', cellWorld: 0.4, cellZ: 0, snapAnchor: 'cross' }), { x: 0.8, z: 0.8 });
});
...
test('snapToCell: hex / off / zero-cell are identity (safe to always call)', () => {
  ...
  assert.deepEqual(snapToCell(3.3, -1.7, { gridStyle: 'square', cellWorld: 0 }), { x: 3.3, z: -1.7 });
