...
test('rate-limit middleware returns 429 with Retry-After and keeps namespaces separate', async () => {
  ...
  const store = {
    async consume(key) {
      keys.push(key);
      return { allowed: false, retryAfterMs: 1200 };
    },
  };
...
test('Redis URL files and trusted proxy hop counts are validated', () => {
  assert.equal(
    redisConnectionUrl({ env: { REDIS_URL: 'redis://cache:6379' } }),
    'redis://cache:6379',
  );
  assert.equal(
    redisConnectionUrl({
      env: { REDIS_URL_FILE: '/secret' },
      readFile: () => 'redis://secret:6379\n',
    }),
    'redis://secret:6379',
  );
...
test('memory fallback is explicit in production and unknown store modes fail startup', async () => {
  ...
  const store = await createRateLimitStore({
    env: { NODE_ENV: 'production', RATE_LIMIT_STORE: 'memory' },
    logger,
  });
  assert.equal(store instanceof MemoryTokenBucketStore, true);
  store.close();
  await assert.rejects(
    createRateLimitStore({ env: { RATE_LIMIT_STORE: 'filesystem' }, logger }),
    /redis or memory/,
  );
});
