...
test('successful absent user reads remain null, empty, and zero', async () => {
  const users = createUserQueries(async (sql) => ({
    rows: sql.includes('count(*)') ? [{ n: 0 }] : [],
  }));
...
test('missing token avoids a database query while a database outage rejects every real read', async () => {
  ...
  let calls = 0;
  const users = createUserQueries(async () => {
    calls++;
    throw outage;
  });
  ...
  for (const read of [
    () => users.findUserByLogin('player'),
    () => users.findUserByToken('hash'),
    () => users.findUserById('1'),
    () => users.listUsers(),
    () => users.countPendingHosts(),
    () => users.roomsOwnedBy('1'),
  ])
