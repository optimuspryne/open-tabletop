...
test('safe message boundary catches rejected handlers and reports sanitized context', async () => {
  ...
  const { invoke, logs, sent } = harness(async () => {
  ...
  assert.deepEqual(sent, [
    {
      type: 'serverError',
      payload: { operation: 'libraryRead', message: 'Server error. Try again.' },
    },
  ]);
...
test('a failed invocation does not disable later room messages', async () => {
  ...
  assert.deepEqual(sent, [
    {
      type: 'assetError',
      payload: { operation: 'libraryRead', message: 'Library unavailable. Try again.' },
    },
  ]);
});
