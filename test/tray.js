...
import { TRAY, trayCenter, trayParts, trayPlace, inTray, dieR, SEAT_ANGLES, seatAngle } from '../shared/pieces.js';
...
function buildSeatTray(world, seat, tableX, tableZ, mat) {
  const angle = seatAngle(seat), center = trayCenter(angle, tableX, tableZ);
  ...
  for (const part of trayParts()) {
    ...
    b.position.set(p.x, p.y, p.z); b.quaternion.copy(spin);
    world.addBody(b);
  }
  return { angle, center };
}
...
test('trayParts: a floor (top at y=0) + four walls + an invisible lid', () => {
  ...
  assert.ok(walls.every(w => Math.abs(w.y - TRAY.wall) < 1e-9), 'walls stand on the floor');
  const lid = parts[5];
  assert.equal(lid.noMesh, true, 'the lid is physics-only (never drawn)');
  assert.ok(Math.abs((lid.y - lid.hy) - 2 * TRAY.wall) < 1e-9, 'lid bottom sits flush with the wall tops');
});
...
test('inTray round-trips trayPlace for interior points at any angle', () => {
  for (const angle of [0, 1.1, 3.9, 5.7]) {
    ...
    for (const [lx, lz] of [[0, 0], [1.5, -1], [-2, 1.5], [TRAY.hx - 0.1, TRAY.hz - 0.1]]) {
...
test('physics: a die dropped into the walled tray stays contained', () => {
  ...
  const angle = 0.8, center = trayCenter(angle, 10, 7);
  const spin = new CANNON.Quaternion(); spin.setFromAxisAngle(new CANNON.Vec3(0, 1, 0), angle);
  for (const part of trayParts()) {                     // build the tray exactly as the server does
  ...
  assert.equal(inTray(die.position.x, die.position.z, center, angle, 0.2), true, 'die stayed within the walls');
  assert.ok(die.position.y > -0.1 && die.position.y < 2 * TRAY.wall + 1, 'die rests on the floor, not through it or over the wall');
});

test('physics: a "Roll all" of several tray dice keeps them all in the tray', () => {
  const world = new CANNON.World({ gravity: new CANNON.Vec3(0, -20, 0) });
  world.broadphase = new CANNON.NaiveBroadphase();
  const angle = 2.1, center = trayCenter(angle, 12, 8);
  const spin = new CANNON.Quaternion(); spin.setFromAxisAngle(new CANNON.Vec3(0, 1, 0), angle);
  for (const part of trayParts()) {
    const b = new CANNON.Body({ mass: 0 });
    b.addShape(new CANNON.Box(new CANNON.Vec3(part.hx, part.hy, part.hz)));
  ...
  const r = dieR(20), dice = [];
  for (let i = 0; i < 5; i++) {                             // a handful of d20-sized dice
    ...
    world.addBody(d); dice.push(d);
  }
  for (let i = 0; i < 300; i++) world.step(1 / 60);         // ~5s to settle after the toss

  for (const d of dice) {
    assert.equal(inTray(d.position.x, d.position.z, center, angle, 0.3), true, 'a rolled die stayed in the tray');
...
test('personal trays: two seats\' dice each stay in their OWN tray', () => {
  ...
  const make = (frame, i) => {
    ...
    d.angularVelocity.set(R.spin, R.spin, R.spin);
    world.addBody(d); return d;
  };
  const aDice = [0, 1, 2].map(i => make(a, i));
  const bDice = [0, 1, 2].map(i => make(b, i));
  for (let i = 0; i < 300; i++) world.step(1 / 60);
  for (const d of aDice) assert.equal(inTray(d.position.x, d.position.z, a.center, a.angle, 0.3), true, 'seat-2 die stayed in seat 2\'s tray');
  for (const d of bDice) assert.equal(inTray(d.position.x, d.position.z, b.center, b.angle, 0.3), true, 'seat-4 die stayed in seat 4\'s tray');
  // and they never wandered into each other's tray
  for (const d of aDice) assert.equal(inTray(d.position.x, d.position.z, b.center, b.angle, 0.3), false, 'seat-2 die did not end up in seat 4\'s tray');
});
