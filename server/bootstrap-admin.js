...
export async function bootstrapAdminFromEnvironment({
  ...
  if (!username || !email || !passwordFile) {
    throw new Error(
      'Admin bootstrap requires BOOTSTRAP_ADMIN_USERNAME, BOOTSTRAP_ADMIN_EMAIL, and BOOTSTRAP_ADMIN_PASSWORD_FILE.',
    );
  }
  if (!validUsername(username))
    throw new Error('BOOTSTRAP_ADMIN_USERNAME must be 3–20 characters (letters, numbers, _ or -).');
  ...
  if (password.length < 12)
    throw new Error('Bootstrap administrator password must be at least 12 characters.');
