export const roomRow = (row) => row && ({
  id: String(row.id), ownerId: String(row.owner_id), code: row.code, name: row.name,
  requireApproval: row.require_approval, createdAt: row.created_at, deletedAt: row.deleted_at,
});
export const memberRow = (row) => row && ({
  roomId: String(row.room_id), userId: String(row.user_id), role: row.role, status: row.status,
});
export const DEFAULT_ROOM_STATE = Object.freeze({
  scoreboard: [], notes: '', tableX: 10, tableZ: 7, skybox: '',
  feltColor: '#2f6b4f', scene: null, scale: null,
});
...
export function createRoomQueries(query) {
  const getMembership = async (roomId, userId) => {
    const { rows } = await query('SELECT * FROM room_members WHERE room_id = $1 AND user_id = $2', [roomId, userId]);
    return memberRow(rows[0]) || null;
  };

  return {
    async findRoomByCode(code) {
      const { rows } = await query('SELECT * FROM rooms WHERE code = $1 AND deleted_at IS NULL', [code]);
    ...
    async listRoomsForAdmin(adminId) {
      ...
      return rows.map((row) => ({ ...roomRow(row), ownerName: row.owner_name, status: 'admitted',
    ...
    async getRoomState(roomId) {
      const { rows } = await query(
        'SELECT scoreboard, notes, table_x, table_z, skybox, felt_color, scene, scale FROM rooms WHERE id = $1', [roomId]);
      if (!rows[0]) return freshDefaultState();
      const row = rows[0];
      return { scoreboard: row.scoreboard || [], notes: row.notes || '', tableX: Number(row.table_x) || 10,
        tableZ: Number(row.table_z) || 7, skybox: row.skybox || '', feltColor: row.felt_color || '#2f6b4f',
        scene: row.scene || null, scale: row.scale || null };
    },
    ...
    async listMembers(roomId) {
      ...
      return rows.map((row) => ({ userId: String(row.user_id), username: row.username,
        avatar: row.avatar, role: row.role, status: row.status }));
    },
  };
}
