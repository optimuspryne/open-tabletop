...
export function createLibraryQueries(query) {
  return {
    async listDecks({ includePrivate = false } = {}) {
      ...
      return rows.map((row) => ({ id: String(row.id), name: row.name, count: Number(row.count), first: row.first || null,
        back: row.back || 'back', isPublic: row.is_public, ownerId: idOrNull(row.owner_id) }));
    },

    async getDeck(id) {
      const { rows } = await query('SELECT name, cards, props, is_public, owner_id FROM custom_decks WHERE id = $1', [id]);
      if (!rows[0]) return null;
      const row = rows[0];
      return { name: row.name, fronts: row.cards || [], back: (row.props || {}).back || 'back',
        geom: (row.props || {}).geom || null, isPublic: row.is_public, ownerId: idOrNull(row.owner_id) };
    },
    ...
    async listBoards({ includePrivate = false } = {}) {
      ...
      return rows.map((row) => {
        ...
        return { ...record, id: String(row.id), name: row.name, kind: boardKind(record),
          preview: row.file_url || (row.props && row.props.tex) || null,
          isPublic: row.is_public, ownerId: idOrNull(row.owner_id) };
      });
    },

    async getBoard(id) {
      const { rows } = await query('SELECT name, file_url, props, is_public, owner_id FROM custom_boards WHERE id = $1', [id]);
      if (!rows[0]) return null;
      const row = rows[0];
      return { rec: boardRecord(row), name: row.name, isPublic: row.is_public, ownerId: idOrNull(row.owner_id) };
    },
    ...
    async listProps({ includePrivate = false } = {}) {
      ...
      return rows.map((row) => ({ id: String(row.id), name: row.name,
        props: { model: row.file_url, ...(row.props || {}) }, isPublic: row.is_public, ownerId: idOrNull(row.owner_id) }));
    },
    ...
    async listScenes({ includePrivate = false } = {}) {
      const { rows } = await query(
        `SELECT id, name, is_public, owner_id FROM custom_scenes
         ${includePrivate ? '' : 'WHERE is_public = true'} ORDER BY name, id`);
      return rows.map((row) => ({ id: String(row.id), name: row.name, isPublic: row.is_public, ownerId: idOrNull(row.owner_id) }));
    },

    async getScene(id) {
      const { rows } = await query('SELECT name, props, is_public, owner_id FROM custom_scenes WHERE id = $1', [id]);
      if (!rows[0]) return null;
      const row = rows[0];
      return { name: row.name, payload: row.props || {}, isPublic: row.is_public, ownerId: idOrNull(row.owner_id) };
    },
    ...
    async listSkyboxes({ includePrivate = false } = {}) {
      const { rows } = await query(
        `SELECT id, name, file_url, is_public, owner_id FROM custom_skyboxes
         ${includePrivate ? '' : 'WHERE is_public = true'} ORDER BY name, id`);
      return rows.map((row) => ({ id: String(row.id), name: row.name, url: row.file_url,
        isPublic: row.is_public, ownerId: idOrNull(row.owner_id) }));
    },
  };
}
