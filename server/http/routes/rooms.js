...
export function createRoomsRouter({
  ...
  router.get(
    ...
    asyncRoute(async (req, res) => {
      ...
      const rooms = user.isAdmin
        ? await db.listRoomsForAdmin(user.id)
        : await db.listRoomsForUser(user.id);
      res.json({ rooms });
    }),
  );
  ...
  router.post(
    ...
    asyncRoute(async (req, res) => {
      ...
      const name =
        String((req.body && req.body.name) || '')
          .trim()
          .slice(0, 60) || 'Untitled Table';
      ...
      if (membership && membership.status === 'pending') {
        try {
          ...
          for (const record of live)
            await matchMaker.remoteRoomCall(record.roomId, 'broadcastMembers');
        } catch {
          /* no live table; GMs see the member on their next refresh */
        }
      }
  ...
  async function ownedRoom(req, res) {
    ...
    if (String(room.ownerId) !== String(user.id) && !user.isAdmin) {
  ...
  router.patch(
    ...
    asyncRoute(async (req, res) => {
      ...
      if (!room) return;
      const name =
        req.body && typeof req.body.name === 'string' ? req.body.name.trim().slice(0, 60) : null;
      if (name) await db.renameRoom(room.id, name);
      if (req.body && typeof req.body.requireApproval === 'boolean')
        await db.setRoomPolicy(room.id, req.body.requireApproval);
