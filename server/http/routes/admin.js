...
export function createAdminRouter({ db, requireAdmin, findOrphanAssets, trashOrphans, disposeLive, kickUserEverywhere }) {
  ...
  router.get('/orphans', asyncRoute(async (req, res) => {
    if (!await requireAdmin(req, res)) return;
    const orphans = await findOrphanAssets();
    res.json({ count: orphans.length, totalBytes: orphans.reduce((sum, item) => sum + item.size, 0),
      files: orphans.slice(0, 200).map((item) => ({ url: item.url, size: item.size })) });
  }));

  router.post('/orphans/purge', asyncRoute(async (req, res) => {
    if (!await requireAdmin(req, res)) return;
    const orphans = await findOrphanAssets();
  ...
  router.get('/pending-count', asyncRoute(async (req, res) => {
    if (!await requireAdmin(req, res)) return;
    res.json({ pending: await db.countPendingHosts() });
  }));

  router.post('/users/:id/host', express.json({ limit: '1kb' }), asyncRoute(async (req, res) => {
    if (!await requireAdmin(req, res)) return;
  ...
  router.post('/rooms/:id/restore', asyncRoute(async (req, res) => {
    if (!await requireAdmin(req, res)) return;
  ...
  router.post('/users/:id/admin', express.json({ limit: '1kb' }), asyncRoute(async (req, res) => {
  ...
  router.post('/users/:id/kick', asyncRoute(async (req, res) => {
    if (!await requireAdmin(req, res)) return;
  ...
  router.delete('/users/:id', asyncRoute(async (req, res) => {
    const actor = await requireAdmin(req, res); if (!actor) return;
    if (String(req.params.id) === String(actor.id)) return res.status(400).json({ error: 'you cannot delete your own account' });
