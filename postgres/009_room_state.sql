-- 009_room_state.sql — durable per-room felt color + the GM's "Save Table State"
-- snapshot, so the felt color persists and pieces survive an empty room.
-- Run as the OWNER role (tabletop), after 008_room_skybox.sql:
--   psql -U tabletop -d tabletop -f 009_room_state.sql

BEGIN;

-- The felt color was already GM-settable but wasn't persisted; give it a home.
