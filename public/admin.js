...
async function loadRooms() {
  ...
  for (const room of rooms) {
    ...
    if (room.deletedAt) tr.className = 'deleted';
    tr.append(
      cell(room.name),
      cell(room.code),
      cell(room.ownerName || '\u2014'),
...
async function loadUsers() {
  ...
  byId('pendingBadge').textContent = pending
    ? `\u2014 ${pending} host request${pending > 1 ? 's' : ''} pending`
    : '';
  for (const user of users) {
    ...
    if (String(user.id) === String(myId)) {
      ...
      you.className = 'muted';
      you.textContent = '(you)';
      acts.appendChild(you);
    } else {
      if (!user.isAdmin) {
        // host queue only applies to non-admins
        if (user.hostStatus === 'pending') {
          acts.append(
            btn('Approve', () => setHost(user, 'approved')),
            btn('Reject', () => setHost(user, 'none'), 'danger'),
          );
      ...
      acts.append(
        btn('Kick', () => kickUser(user)),
        btn('Delete', () => deleteUser(user), 'danger'),
      );
    }
    tr.append(cell(acts));
    tbody.appendChild(tr);
...
async function setHost(user, status) {
  if (
    status === 'none' &&
    !confirm(
      `${user.hostStatus === 'pending' ? 'Reject' : 'Revoke host access for'} ${user.username}?`,
    )
  )
    return;
  try {
    await api('/admin/users/' + user.id + '/host', { method: 'POST', body: { status } });
    loadUsers();
  } catch (e) {
    alert(e.message);
  }
...
async function renameRoom(room) {
  ...
  try {
    await api('/rooms/' + room.id, { method: 'PATCH', body: { name: name.trim() } });
    loadRooms();
  } catch (e) {
    alert(e.message);
  }
...
async function closeRoom(room) {
  if (
    !confirm(
      `Close "${room.name}"? Anyone at the table is sent to the lobby; it becomes unjoinable.`,
    )
  )
    return;
  try {
...
async function restoreRoom(room) {
  try {
    await api('/admin/rooms/' + room.id + '/restore', { method: 'POST' });
    loadRooms();
  } catch (e) {
    alert(e.message);
  }
...
async function purgeRoom(room) {
  if (!confirm(`Permanently delete "${room.name}" and all its memberships? This cannot be undone.`))
    return;
  try {
...
async function kickUser(user) {
  try {
    ...
    alert(
      rooms
        ? `Dropped ${user.username} from ${rooms} live table${rooms > 1 ? 's' : ''}.`
        : `${user.username} isn't in any live table.`,
    );
  } catch (e) {
    alert(e.message);
  }
...
async function deleteUser(user) {
  if (
    !confirm(
      `Permanently delete ${user.username}? This purges any rooms they own and removes them from all rooms. This cannot be undone.`,
    )
  )
    return;
  try {
...
async function scanOrphans() {
  const resultEl = byId('cleanupResult');
  resultEl.textContent = 'Scanning…';
  try {
    const { count, totalBytes } = await api('/admin/orphans');
    if (!count) {
      resultEl.textContent = 'No orphaned files found.';
      return;
    }
    ...
    resultEl.append(
      label,
      btn(`Move ${count} to trash`, () => purgeOrphans(count), 'danger'),
    );
  } catch (e) {
...
async function purgeOrphans(count) {
  if (
    !confirm(
      `Move ${count} orphaned file(s) to saved-assets/.trash/? They stay recoverable there until you delete the folder.`,
    )
  )
    return;
  const resultEl = byId('cleanupResult');
