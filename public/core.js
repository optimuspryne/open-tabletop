...
// ===== scene (visual only — the server owns all physics) ====================
// ===== CONFIG — client-side tunables (mirror of the server's SIM block) =====
...
// ===== LIGHTING — tweak these three numbers to taste =======================
const LIGHTING = {
  hemi: 0.30,  // soft sky/ground fill (ambient)
  sun:  2.50,  // main directional light + shadows
  env:  0.45,  // environment-map reflection strength (0 = off/flat, 1 = full studio)
};
const dimEnv = (e, k) => { e.traverse(o => { if (o.isLight) o.intensity *= k; if (o.material && o.material.color) o.material.color.multiplyScalar(k); }); return e; };
...
// A neutral studio environment so PBR/metallic materials (incl. custom .glb) reflect
// something instead of rendering dark. Subtle — mostly helps metals read as material.
const _pmrem = new THREE.PMREMGenerator(renderer);
scene.environment = _pmrem.fromScene(dimEnv(new RoomEnvironment(), LIGHTING.env), 0.04).texture;
...
sun.position.set(10,18,8); sun.castShadow = true; sun.shadow.mapSize.set(2048,2048);
Object.assign(sun.shadow.camera, { left:-14, right:14, top:12, bottom:-12 }); scene.add(sun);
const tableMesh = new THREE.Mesh(new THREE.BoxGeometry(TABLE.x*2,1,TABLE.z*2),
  new THREE.MeshStandardMaterial({ color:0x2f6b4f, roughness:0.95 }));
tableMesh.position.y = -0.5; tableMesh.receiveShadow = true; scene.add(tableMesh);
