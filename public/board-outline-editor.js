...
export function wireBoardOutline(prefix) {
  ...
  function draw() {
    cut.parentElement.hidden = select.value !== 'clipped';
    ...
    const outline = select.value === 'custom' ? points : boardOutlinePoints(value());
    ctx.beginPath();
  ...
  return {
    ...
    read() {
      const result = value();
      if (!result) throw new Error('Draw a convex outline with 3–32 corners in edge order.');
      return result;
    },
    fill(outline) {
